...
export default function PhotoCard({ photo, onActionComplete }: PhotoCardProps) {
  ...
  return (
    <Card sx={{ position: "relative", overflow: "hidden", boxShadow: 3, minWidth: 280 }}>
      {!imageLoaded && (
        <Box
          sx={{
            height: 200,
      ...
      <CardContent>
        <Typography variant="subtitle1"><strong>Photo Name:</strong> {photo.name}</Typography>
        <Typography variant="body2"><strong>Caption:</strong> {photo.attributes?.caption || "No caption available"}</Typography>
        <Typography variant="body2"><strong>Uploaded by:</strong> {photo.attributes?.uploadedBy || "Unknown"}</Typography>
        <Typography variant="body2">
          <strong>Created at:</strong> {new Date(photo.attributes?.createdAt ?? Date.now()).toLocaleDateString()}
        </Typography>
        ...
        <Typography variant="body2" sx={{ marginTop: 1 }}>
          <strong>Tags:</strong> {photo.tags.length > 0 ? photo.tags.join(', ') : "No tags available"}
        </Typography>
